...
(function (global) {
  ...
  const BODY = Object.freeze({HeadAngleZ:0,Tilt:0,Sway:0,Bob:0,Torso:0,TorsoSway:0,Crouch:0,ArmL:0,ArmR:0,ArmBeat:0,CheekReach:0,MouthReach:0,Wave:0,FootStep:0,HairSwing:0,Sob:0,TailSwing:0,Tempo:0,Stance:0,TailDroop:0,TailFlick:0,ShoulderDrop:0,Twist:0,TailStraight:0,Nod:0,Scan:0,Exhale:0,RandomTilt:0,Glance:0});
  const BODY_RANGES = {HeadAngleZ:[-8,8],Tilt:[-8,8],Sway:[0,2],Bob:[0,3],Torso:[-3,3],TorsoSway:[0,2],Crouch:[0,8],ArmL:[-18,45],ArmR:[-18,45],ArmBeat:[0,12],CheekReach:[0,1],MouthReach:[0,1],Wave:[0,1],FootStep:[0,5],HairSwing:[0,4],Sob:[0,1],TailSwing:[0,5],Tempo:[0,8],Stance:[-1,1],TailDroop:[0,1],TailFlick:[0,1],ShoulderDrop:[0,14],Twist:[0,1],TailStraight:[0,1],Nod:[0,1],Scan:[0,1],Exhale:[0,1],RandomTilt:[0,1],Glance:[0,1]};
  ...
  class MintRig {
    constructor(host, sourceSvg, rig, expressions, options = {}) {
      ...
      this.options = {idle: true, tracking: true, poseTracking: false, reducedMotion: false, ...options};
      ...
      this.gaze = {x: 0, y: 0, targetX: 0, targetY: 0, responseSeconds:.12};this.poseFollow={x:0,y:0,targetX:0,targetY:0,responseSeconds:.22}; this.manualBlinkAt = -100;
      ...
      this.speech={enabled:false,target:0,value:0,mix:0,vowel:null,gain:.9,vowelOpen:1,vowelRound:0};this.symbols=new MintEmotionEffects(this);this.interactions=new MintInteractions(this);this.performance=new MintPerformance(this);
      this.render();
    ...
    directInteraction(kind,side='left',amount=1){this.interactions.directTo(kind,side,amount);}
    directCheekPull(side,amount,dx,dy){this.interactions.directPull(side,amount,dx,dy);}
    setInteractionPointer(clientX,clientY,active=true,kind=null){this.interactions.setPointer(clientX,clientY,active,kind);}
    stopInteraction(duration=.24){this.interactions.stop(duration);this.render();}
    ...
    _renderTorsoMesh(d){
      const base=this.frameBase,shape=this.frameSquash,puff=base.CheekPuff,pull=base.CheekPull;
      ...
      const pullDeform=base.CheekPullDeform??pull;
      this.faceMesh.mesh.deform((p,w)=>{
        const side=this.interactions?.pullSide||-1,deform=pullDeform,grip=[side<0?326:742,690];
        const sideGate=smooth(-15,195,side*(p[0]-534)),radial=Math.exp(-(((p[0]-grip[0])/255)**2)-(((p[1]-grip[1])/155)**2));
        const influence=radial*(.06+.94*sideGate)*(.18+.82*w.cheek),vx=(base.CheekPullX??side*165)*deform,vy=(base.CheekPullY??0)*deform;
        const squash=-(p[1]-grip[1])*.065*Math.abs(deform)*influence;
        return[p[0]+(p[0]-534)*.055*puff*w.cheek+vx*influence,p[1]-4*puff*w.cheek+vy*influence+squash];
      });
      this._writeMeshPaths(this.faceMesh.mesh,this.faceMesh.records,puff===0&&pull===0&&Math.abs(pullDeform)<1e-8);
    }
    _renderTailMesh(d){
      ...
      const tapTime=this.clock%4.8;
      const tap=(t)=>t<0||t>1.15?0:t<.32?-3*smooth(0,.32,t):t<.5?lerp(-3,12,smooth(.32,.5,t)):12*(1-smooth(.5,1.15,t));
      const flick=motion*body.TailFlick*(tap(tapTime-.4)+.72*tap(tapTime-1.55));
      ...
      const root=[675,1120],mid=[790,1180],rootAngle=swing+droop*43+straight*25+flick*.16,tipAngle=(1-straight)*(droop*4+flick);
      ...
      this.tailMesh.mesh.deform((p,w)=>{
        ...
        return[p[0]+(b[0]-p[0])*pin+straight*(p[0]-675)*.045,
    ...
    _renderFullBody(d,headLocal){
      ...
      const crouch=body.Crouch+motion*body.Sob*(1+fast)*1.5+(124-Math.sqrt(124*124-(Math.max(0,body.Stance)*45)**2)-Math.sin(Math.max(0,body.Stance)*9*Math.PI/180)*82-(Math.cos(Math.max(0,body.Stance)*9*Math.PI/180)-1)*153+Math.max(0,body.Stance)*4)+Math.abs(Math.sin(angle*Math.PI/180))*83;
      ...
      this.legData.forEach((leg,i)=>{
        ...
        const rest=Math.abs(angle)+Math.abs(crouch)+lift+Math.abs(stance)<1e-9;
        ...
        leg.mesh.deform((p,w)=>{if(rest)return [...p];const x=p[0]-leg.hip[0],y=p[1]-leg.hip[1],t=(x*ux+y*uy)/restLength,width=-x*uy+y*ux,soft=Math.sin(Math.PI*clamp(t,0,1))**2*.42;
          ...
          const upperSkin=map(p,leg.hip,hip,upper),lowerSkin=map(p,leg.knee,knee,lower),boneMix=.16*Math.sin(Math.PI*clamp(t,0,1))**2*(1-distal);
          for(let k=0;k<2;k++)shaped[k]=lerp(shaped[k],upperSkin[k]*w.upper+lowerSkin[k]*w.lower,boneMix);
          const kneeBulge=4*Math.sin(Math.PI*clamp(t,0,1))**2*Math.min(1,Math.abs(upper-lower)/35);
          shaped[0]+=Math.sign(p[0]-leg.hip[0])*kneeBulge*(1-distal);
    ...
    setEmotion(id,intensity=1,duration=this.config.transition.durationSeconds) {
      ...
      const hood=Object.hasOwn(this.manual,'HoodOff')?this.manual.HoodOff:(this.target.HoodOff||0);
      this.emotion=id; this.intensity=intensity; this.manual={HoodOff:hood}; this._retarget({...this._expressionTarget(),...this.manual},duration);
    }
    ...
    setGaze(x,y,responseSeconds=.12) {
      if(!Number.isFinite(x)||!Number.isFinite(y)) throw new Error('Nonfinite gaze');
      if(!Number.isFinite(responseSeconds)||responseSeconds<.04||responseSeconds>1)throw new Error('Invalid gaze response');
      this.gaze.targetX=clamp(x,-1,1);this.gaze.targetY=clamp(y,-1,1);this.gaze.responseSeconds=responseSeconds;
    }
    gazeFromClient(clientX,clientY,responseSeconds=.12) {
      const m=this.headSlots[1].getScreenCTM(); if(!m)return;
      const point=new DOMPoint(clientX,clientY).matrixTransform(m.inverse());
      this.setGaze((point.x-534)/240,(point.y-610)/210,responseSeconds);
    }
    poseFromClient(clientX,clientY,responseSeconds=.22){const m=this.headSlots[1].getScreenCTM();if(!m)return;const p=new DOMPoint(clientX,clientY).matrixTransform(m.inverse());this.poseFollow.targetX=clamp((p.x-534)/300,-1,1);this.poseFollow.targetY=clamp((p.y-610)/260,-1,1);this.poseFollow.responseSeconds=clamp(responseSeconds,.08,.8)}
    ...
    setPoseTracking(enabled){this.options.poseTracking=Boolean(enabled);if(!enabled){this.poseFollow.targetX=0;this.poseFollow.targetY=0}}
    ...
    reset({canonical=false}={}) {
      this.performance?.reset();this.interactions?.reset();this.speech={enabled:false,target:0,value:0,mix:0,vowel:null,gain:.9,vowelOpen:1,vowelRound:0};this.parts?.reset();this.clock=0;this.beatPhase=0;this.emotion='neutral';this.intensity=1;this.manual={};this.target={...this.defaults};
      ...
      if(canonical){this.options.idle=false;this.options.tracking=false;this.options.poseTracking=false;this.paused=true;}
    ...
    _drivers(base) {
      ...
      const body=this._sampleBody(),random=this.performance?.random(this.clock)||{tilt:0,glance:0},phase=Math.sin(this.beatPhase),motionGain=reduced?0:1;
      const clip=this.clock%6.4,window=smooth(.3,.8,clip)*(1-smooth(2.6,3.3,clip));
      const shake=motionGain*body.Nod*window*Math.sin((clip-.4)*5)*2.6,scan=motionGain*body.Scan*random.lookX,exhale=motionGain*body.Exhale*smooth(.5,1.5,clip)*(1-smooth(2,3.8,clip));
      const scanHead=motionGain*body.Scan*random.lookX;body.ShoulderDrop+=exhale*2;
      body.Tilt+=motionGain*body.RandomTilt*random.tilt;
      body.Tilt+=motionGain*(this.interactions?.sample().pet||0)*1.6*Math.sin(this.clock*2.8-.3);
      const pose=this.options.poseTracking?this.poseFollow:{x:0,y:0};
      const bodyAngle=body.Tilt+motionGain*body.Sway*phase+pose.x*1.8,bodyY=motionGain*body.Bob*Math.sin(this.beatPhase-.38)*4+exhale*5+pose.y*2.2;
      const contact=this.interactions?.sample()||{poke:0,left:0,right:0};const pointerGain=(this.options.tracking?1:0)*(1-contact.poke)*(1-body.Glance),directed=(contact.right-contact.left)*.85;
      return {
        breath,bodyAngle,bodyY,body,
        head:clamp(base.HeadAngleZ+body.Tilt+shake+scanHead*2+motionGain*body.Sway*Math.sin(this.beatPhase-.22)+pointerGain*this.gaze.x*2.2+pose.x*2.6+gain*.28*Math.sin(this.clock*.79),-8,8),
        gazeX:clamp((base.EyeBallX*(1-body.Glance)+scan*.65+motionGain*body.Glance*random.glance*.65)*(1-contact.poke)+directed+pointerGain*this.gaze.x+gain*.06*Math.sin(this.clock*.61)*(1-contact.poke),-1,1),
        gazeY:clamp((base.EyeBallY*(1-body.Glance)+body.Glance*.65+motionGain*body.Scan*random.lookY*.75)*(1-contact.poke)+pointerGain*this.gaze.y+gain*.04*Math.sin(this.clock*.83)*(1-contact.poke),-1,1),
    ...
    _step(dt) {
      ...
      const response=1-Math.exp(-dt/this.gaze.responseSeconds);
      this.gaze.x=lerp(this.gaze.x,this.gaze.targetX,response);this.gaze.y=lerp(this.gaze.y,this.gaze.targetY,response);
      const poseResponse=1-Math.exp(-dt/this.poseFollow.responseSeconds);this.poseFollow.x=lerp(this.poseFollow.x,this.poseFollow.targetX,poseResponse);this.poseFollow.y=lerp(this.poseFollow.y,this.poseFollow.targetY,poseResponse);
    ...
    _renderMouth(base) {
      ...
      top=top.map((v,i)=>i%2?lerp(v,683+(v-683)*.3,base.MouthFlat):v);
      this._set(this.lip,'d',f===0&&open===0&&base.MouthFlat===0?this.rest.get(this.lip).d:pathFromLip(top));
