 ...
 class MintPartVariants{
   ...
   for(const kind of ['arm'])for(const side of ['left','right']){
    ...
    for(const name of names.slice(1)){
     ...
     const mesh=r._newPartMesh('variant-'+slot+'-'+name,frame,[...frame.querySelectorAll('path')],{columns:8,rows:8,weights:p=>{
      if(name==='peace'||name==='clasp'||name==='open'){
  ...
  render(d){
   ...
   for(const e of this.entries){
    ...
    const indexHand=e.name==='index'?e.frame.querySelector('[id$="-hand"]'):null;
    const fingerGap=e.name==='index'?motion*d.body.Glance*6*(1-Math.cos(r.clock*1.7))*.5:0;
    ...
    const indexBone=e.name==='index'?motion*d.body.Glance*(e.side==='left'?1:-1)*(1.8*Math.sin(r.clock*1.7-.35)+.7*Math.sin(r.clock*3.4)):0;
    const angle=motion*(['wave','peace'].includes(e.name)?(1.2+d.body.Wave*2)*Math.sin(phase*2.5):e.kind==='leg'?d.body.FootStep*.65*Math.sin(phase):pulse)+indexBone;
    const finger=e.name==='index'?motion*(d.body.Glance?4.5:1.2)*(1-Math.cos(r.clock*1.7))*.5*(e.side==='left'?-1:1):0;
    e.mesh.deform((p,w)=>{
     if(e.name==='peace'||e.name==='clasp'||e.name==='open'){
      const sign=e.side==='left'?1:-1,clasp=e.name==='clasp',open=e.name==='open',amp=motion*(open?Math.min(1,d.body.ArmBeat/6):clasp?d.body.Twist:d.body.Wave),upper=sign*amp*(open?3.5:clasp?1.4:3)*Math.sin(phase*(clasp?.65:.7)),lower=sign*amp*(open?6:clasp?-1.1:15)*Math.sin(phase*(open?.85:clasp?.65:1.8)),hand=sign*amp*(open?3:clasp?.4:11)*Math.sin(phase*(open?.85:clasp?.65:1.8)-.55);
      ...
      e.bonePose={upper,lower,hand,elbow:joint,wrist:wpos};return[0,1].map(k=>p[k]*w.root+a[k]*w.upper+b[k]*w.lower+c[k]*w.hand+(open?(k===1?Math.sin(phase-.4)*3:-sign*Math.sin(phase-.7)*2)*4*w.upper*w.lower:0));
     }
     ...
     return[p[0]+(q[0]-p[0]+delta[0]+finger)*w.tip+(p[0]-e.root[0])*volume,
      p[1]+(q[1]-p[1]+delta[1]+pulse+(e.name==='index'?Math.sin(r.clock*1.7)*2.4*d.body.Glance:0))*w.tip-(e.kind==='leg'?r.frameSquash*8*w.tip:0)];
    });
    r._writeMeshPaths(e.mesh,e.records);
    if(indexHand)r._set(indexHand,'transform',fingerGap?`translate(${-fingerGap} 0)`:null);
    if(r.rigDebug.mesh||r.rigDebug.bones){
     ...
     r._set(e.bones,'d',['peace','clasp','open'].includes(e.name)&&e.bonePose?line(e.root,e.bonePose.elbow)+line(e.bonePose.elbow,e.bonePose.wrist)+line(e.bonePose.wrist,e.mesh.sample(e.mesh.bind(e.tip))):line(e.root,e.mesh.sample(e.mesh.bind(e.tip))));
    }
   }
  }
